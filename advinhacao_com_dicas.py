...
def jogar():
    ...
    def chute_maior(): 
        if tentativa_do_usuario > numero_sorteado: 
            return print("você chutou um numero maior que o sorteado")

    def chute_menor():
        if tentativa_do_usuario < numero_sorteado: 
            return print("você chutou um numero menor que o sorteado")
    ...
    while True: 
        ...
        if numero_sorteado == tentativa_do_usuario:
            print(f'Parabéns! Você acertou o número secreto! Você levou {rodada} rodadas para acertar e obteve {pontuação(rodada, nivel)} pontos')
            break
